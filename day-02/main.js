...
function parseInput(inputText) {
  return inputText
    ...
    .map((string) => {
      ...
      return {
        min: parseInt(array[0].split("-")[0]),
        max: parseInt(array[0].split("-")[1]),
        char: array[1].split(":")[0],
        pass: array[2],
        valid: null,
      };
    });
...
function parseInput2(inputText) {
  return inputText
    ...
    .map((string) => {
      const array = string.split(" ");
      return {
        first: parseInt(array[0].split("-")[0]),
        second: parseInt(array[0].split("-")[1]),
        char: array[1].split(":")[0],
        pass: array[2],
        valid: null,
      };
...
function resolve1(input) {
  ...
  .map((policy) => {
    ...
    const count = policy.pass.match(regexp) ? policy.pass.match(regexp).length : 0;
    
    policy.valid = policy.min <= count && count <= policy.max;
    return policy;
  })
...
function resolve2(input) {
  return input
  .map((policy) => {
    const p1 = policy.pass.substr(policy.first-1, 1) === policy.char;
    const p2 = policy.pass.substr(policy.second-1, 1) === policy.char;

    policy.valid = ((p1 && !p2) || (!p1 && p2));
    return policy;
  })
  .filter(policy => policy.valid)
  .length;
